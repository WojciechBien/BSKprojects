...
namespace bsk_zadania
{
    public class projekt_4
    {
        ...
        private List<int> hashLenght;
        public bool IsHashSizeConst;
        public string IsHashSizeConstMsg;
        public projekt_4()
        {
            random = new Random();
            ListOfResults = new List<double>();
            hashLenght = new List<int>();
        }
        ...
        public void process(int iterations)
        {
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                using (SHA256 sha256Hash = SHA256.Create())
                {
                    ...
                    hashLenght.Add(hashFromRawData.Length);
                };
            }

            if (hashLenght.GroupBy(x=>x.ToString()).ToList().Count>1)
            {
                IsHashSizeConst = false;
                IsHashSizeConstMsg = "No, hash lengths differs";
            }
            else
            {
                IsHashSizeConst = true;
                IsHashSizeConstMsg = "yes, length is" +
                                     hashLenght[0];
            }
            timer.Stop();
        ...
        private double CalculateStandardDeviation(double[] doubles)
        {
            double average = doubles.Average();
            double sumOfSquaresOfDifferences = doubles.Select(val => (val - average) * (val - average)).Sum();
            return Math.Sqrt(sumOfSquaresOfDifferences / doubles.Length);
        }
